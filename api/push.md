...
`message.is_read` on **incoming** rows is the source for `Chat.unreadCount`. Outgoing `is_read` is a recipient receipt and must not be counted. Missing `is_read` yields `null`. The browser uses this only to initialize its [local read cursor](web/README.md); push still uses Mac unread state. No API route marks read, and opening a conversation issues reads only. Conversation-level “Mark as Unread” and other undocumented schema details are not guaranteed to match these message-level counts.
