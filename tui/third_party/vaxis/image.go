...
func (k *KittyImage) Draw(win Window) {
	k.draw(win, image.Rectangle{}, false)
}

// DrawCrop draws src at the window origin in native uploaded-image pixels.
// Call Resize before drawing. src is clipped to the encoded image and the
// window; partial cells are not stretched. An empty crop adds no placement.
func (k *KittyImage) DrawCrop(win Window, src image.Rectangle) {
	k.draw(win, src, true)
}

func (k *KittyImage) draw(win Window, src image.Rectangle, cropped bool) {
	...
	w, h := k.w, k.h
	if cropped {
		if win.Width <= 0 || win.Height <= 0 || k.cellPixW <= 0 || k.cellPixH <= 0 || col < 0 || row < 0 {
			return
		}
		src = src.Intersect(k.pixels)
		src.Max.X = min(src.Max.X, src.Min.X+win.Width*k.cellPixW)
		src.Max.Y = min(src.Max.Y, src.Min.Y+win.Height*k.cellPixH)
		if src.Empty() {
			return
		}
		w = (src.Dx() + k.cellPixW - 1) / k.cellPixW
		h = (src.Dy() + k.cellPixH - 1) / k.cellPixH
	}
	...
	writeFunc := func(w io.Writer) {
		...
		if cropped {
			_, _ = fmt.Fprintf(w, "\x1B_Ga=p,i=%d,p=%d,C=1,x=%d,y=%d,w=%d,h=%d\x1B\\", k.id, pid, src.Min.X, src.Min.Y, src.Dx(), src.Dy())
		} else {
...
func (k *KittyImage) Resize(w int, h int) {
	...
	// Reupload the image
	k.pixels = image.Rect(0, 0, img.Bounds().Dx(), img.Bounds().Dy())
	max := img.Bounds().Max
	k.w = max.X / cellPixW
	if max.X%cellPixW != 0 {
		k.w += 1
