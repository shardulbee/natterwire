...
The [terminal client](tui/README.md) browses chats and [sends text](api/README.md#sending) to existing conversations through Messages.app.
...
Natterwire opens `chat.db` read-only and sends text through AppleScript. It binds only to loopback and trusts local processes. Tailscale policy authorizes remote reads and sends through Tailscale Serve; restrict Mac TCP 8741 to intended personal devices. There is no application-level authentication. Never use a LAN bind or Tailscale Funnel.
